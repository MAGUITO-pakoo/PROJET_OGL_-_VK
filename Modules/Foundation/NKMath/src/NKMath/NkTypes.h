...
namespace nkentseu {
    ...
    struct NkVec3fHomogeneous {
        float32 x = 0.f, y = 0.f, z = 0.f;

        NkVec3fHomogeneous() = default;
        NkVec3fHomogeneous(float32 x, float32 y, float32 z) : x(x), y(y), z(z) {}
        explicit NkVec3fHomogeneous(const NkVec2f& v, float32 z = 1.f) : x(v.x), y(v.y), z(z) {}

        NkVec2f ToVec2()            const { return { x, y }; }
        NkVec3fHomogeneous operator+(const NkVec3fHomogeneous& o) const { return { x + o.x, y + o.y, z + o.z }; }
        NkVec3fHomogeneous operator*(float32 s)          const { return { x * s, y * s, z * s }; }
    };
